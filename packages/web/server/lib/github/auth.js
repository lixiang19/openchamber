...
const STORAGE_DIR = OPENAURORA_DATA_DIR;
const STORAGE_FILE = path.join(STORAGE_DIR, 'github-auth.json');
const SETTINGS_FILE = path.join(OPENAURORA_DATA_DIR, 'settings.json');
...
function writeJsonFile(payload) {
  ...
  // Atomic write so multiple OpenAurora instances can safely share the same file.
...
export function getGitHubClientId() {
  const raw = process.env.OPENAURORA_GITHUB_CLIENT_ID;
...
export function getGitHubScopes() {
  const raw = process.env.OPENAURORA_GITHUB_SCOPES;
