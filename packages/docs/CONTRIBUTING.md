...
This package is docs content source-of-truth for OpenAurora.
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access OpenAurora from outside your local network.
   ---
   ```
...
## Sync into openaurora-website

`openaurora-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `openaurora-website/apps/docs/src/content/docs/*`
2. map `packages/docs/sidebar.config.json` into `openaurora-website/apps/docs/astro.config.mjs` sidebar
