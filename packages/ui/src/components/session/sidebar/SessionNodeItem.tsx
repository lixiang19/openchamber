    ...
    <DropdownMenuContent align="end" className="min-w-[180px]" onCloseAutoFocus={(event) => { if (renamingFolderId) event.preventDefault(); }}>
      ...
      {!session.share ? (
        ...
        <>
          <DropdownMenuItem onClick={() => { if (session.share?.url) handleCopyShareUrl(session.share.url, session.id); }} className="[&>svg]:mr-1">
            {copiedSessionId === session.id ? <><RiCheckLine className="mr-1 h-4 w-4" style={{ color: 'var(--status-success)' }} />已复制</> : <><RiFileCopyLine className="mr-1 h-4 w-4" />复制链接</>}
          </DropdownMenuItem>
      ...
      {sessionDirectory && !archivedBucket ? (() => {
        ...
        return (
          <>
            <DropdownMenuSeparator />
            <DropdownMenuSub>
              <DropdownMenuSubTrigger className="[&>svg]:mr-1"><RiFolderLine className="h-4 w-4" />移动到文件夹</DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="min-w-[180px]">
                {scopeFolders.length === 0 ? (
                  <DropdownMenuItem disabled className="text-muted-foreground">还没有文件夹</DropdownMenuItem>
                ) : (
      ...
      {!isVSCode ? (
        ...
        >
          ...
          <span className="truncate">在侧边栏面板中打开</span>
          <span className="shrink-0 typography-micro px-1 rounded leading-none pb-px text-[var(--status-warning)] bg-[var(--status-warning)]/10">测试版</span>
        </DropdownMenuItem>
      ) : null}
                ...
                <TooltipTrigger asChild>
                  ...
                  >
                    <div className={cn('flex w-full items-center min-w-0 flex-1 overflow-hidden', isMinimalMode ? 'gap-1' : 'gap-1')}>
                      {isPinnedSession ? <RiPushpinLine className="h-3 w-3 flex-shrink-0 text-primary" aria-label="已置顶会话" /> : null}
