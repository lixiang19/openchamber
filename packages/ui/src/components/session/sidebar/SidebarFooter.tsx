...
export function SidebarFooter({
  ...
  return (
    <div className="flex shrink-0 items-center justify-start gap-1 px-2.5 py-2">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button type="button" onClick={onOpenAbout} className={footerButtonClassName} aria-label="关于 OpenAurora">
            <RiInformationLine className="h-4.5 w-4.5" />
          </button>
        </TooltipTrigger>
        <TooltipContent side="top" sideOffset={4}><p>关于 OpenAurora</p></TooltipContent>
      </Tooltip>
    </div>
  );
}
