...
function App({ apis }: AppProps) {
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'ridge:embedded-visibility') {
        return;
      }
    ...
    const scopedWindow = window as unknown as {
      __ridgeSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__ridgeSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__ridgeSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__ridgeSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __ridgeAppReady?: boolean }).__ridgeAppReady = true;
    window.dispatchEvent(new Event('ridge:app-ready'));
  ...
  if (isVSCodeRuntime) {
    ...
    const panelType = typeof window !== 'undefined' 
      ? (window as { __RIDGE_PANEL_TYPE__?: 'chat' | 'agentManager' }).__RIDGE_PANEL_TYPE__ 
      : 'chat';
