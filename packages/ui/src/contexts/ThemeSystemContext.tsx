...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __RIDGE_VSCODE_THEME__?: Theme }).__RIDGE_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('ridge:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('ridge:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __ridgeApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__ridgeApplyThemeSync = applyIncomingThemeSync;

    return () => {
      if (scopedWindow.__ridgeApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__ridgeApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'ridge:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('ridge:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('ridge:settings-synced', handleSettingsSynced);
  }, []);
