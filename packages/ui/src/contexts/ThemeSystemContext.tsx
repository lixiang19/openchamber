...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __OPENAURORA_VSCODE_THEME__?: Theme }).__OPENAURORA_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('openaurora:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('openaurora:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __openauroraApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__openauroraApplyThemeSync = applyIncomingThemeSync;

    return () => {
      if (scopedWindow.__openauroraApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__openauroraApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'openaurora:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('openaurora:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('openaurora:settings-synced', handleSettingsSynced);
  }, []);
