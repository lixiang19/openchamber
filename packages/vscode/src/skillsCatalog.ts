  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'OpenAurora-VSCode/1.0',
        ...options?.headers,
      },
    });
...
export async function scanSkillsRepository(options: { source: string; subpath?: string; defaultSubpath?: string }): Promise<SkillsRepoScanResult> {
  ...
  const tempBase = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'openaurora-vscode-skills-scan-'));
...
export async function installSkillsFromRepository(options: {
  ...
  const tempBase = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'openaurora-vscode-skills-install-'));
