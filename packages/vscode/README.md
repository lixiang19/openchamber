...
![VS Code Extension](https://github.com/btriapitsyn/openaurora/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/btriapitsyn/openaurora) with even more features.**
...
Plus everything from the shared OpenAurora UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `OpenAurora: Focus Chat` | Focus the chat panel |
| `OpenAurora: New Session` | Start a new chat session |
| `OpenAurora: Open Sidebar` | Open the OpenAurora sidebar |
| `OpenAurora: Open Agent Manager` | Launch parallel multi-model runs |
| `OpenAurora: Open Session in Editor` | Open current or new session in an editor tab |
| `OpenAurora: Settings` | Open extension settings |
| `OpenAurora: Restart API Connection` | Restart the OpenCode API process |
| `OpenAurora: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **OpenAurora** submenu:
...
|---------|---------|-------------|
| `openaurora.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `openaurora.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
Install locally: `code --install-extension packages/vscode/openaurora-*.vsix`
