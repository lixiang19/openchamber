...
const CLI_ARGS_ENV = process.env.OPENAURORA_OPENCODE_ARGS;
...
async function resolveCliPath() {
  ...
  throw new Error('Unable to locate the OpenCode CLI. Set OPENAURORA_OPENCODE_PATH to the executable.');
}
...
function spawnCli(cliPath, args) {
  const env = {
    ...process.env,
    OPENAURORA_OPENCODE_PORT: process.env.OPENAURORA_OPENCODE_PORT || process.env.OPENCODE_PORT || process.env.OPENAURORA_INTERNAL_PORT || '0',
  };
  const cwd = process.env.OPENAURORA_OPENCODE_CWD || process.cwd();
