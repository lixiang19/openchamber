#!/usr/bin/env bash
# OpenAurora Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/openaurora/main/scripts/install.sh | bash

set -e

PACKAGE_NAME="@openaurora/web"
MIN_NODE_VERSION=20
...
main() {
  ...
  # Install
  echo ""
  info "Installing OpenAurora..."
  echo "  Running: $INSTALL_CMD"
  echo ""
  
  if eval "$INSTALL_CMD"; then
    echo ""
    success "OpenAurora installed successfully!"
    echo ""
    echo "  Get started:"
    echo "    openaurora              # Start server on port 3000"
    echo "    openaurora --help       # Show all options"
    echo ""
    echo "  Prerequisites:"
